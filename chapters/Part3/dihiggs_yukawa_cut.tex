...
\begin{figure}[htpb!]
	...
	\caption{The  68\%, 95\% and 99.7\%  HDP contours, for Bayesian fits preformed on pairs of Wilson coefficients for $C_\phi$, $C_{u\phi}$ and $C_{d\phi}$ form the multi-variate analysis output.}
	\label{fig:constraint2d}
\end{figure}
\begin{figure}[t!]
	\centering
	\includegraphics[width =0.65\linewidth]{fig/kappa_u-kappa_d-kappa_l-HL-LHC.pdf}
	\caption{ Three parameter Bayesian fits with $C_{u\phi}$, $C_{d\phi}$ and $C_\phi$, the HDP contours are the same as~\autoref{fig:constraint2d} .}
...
\begin{figure}[!t]
	\centering
	\includegraphics[width = 0.25\textwidth]{./fig/2HDM-bound_full}
	\hspace{0.5 cm}
...
\la{
	The contributions to EWPO's of VLQ model can be parametrized in the SMEFT language in terms of the Wilson coefficients $C_{\phi q}^{(1)/(3)}$ and $C_{\phi G}$, which were investigated in ref.~\cite{Dawson:2020oco}. In VLQ models, the values of the light Yukawa modifiers is related to $C_{\phi q}$ by the SM Yukawa matrices.  $C_{q\phi} = -y_q C_{\phi q}$. Since the only possible contribution to a tree-level EWPO generated by the VLQ's is the $Z \to b \bar{b}$ decay, i.e. from $C_{\phi b}$, this contribution in the AFV framework is either vanishing or at most CKM-suppressed. Making the VLQ's contribution to this decay's width negligible. The other contribution of the VLQ model to the EWPO stems from loop induced $ggh$ vertex, producing $C_{\phi G}$ at a loop-level. However, this is suppressed by the SM light Yukawa couplings, as well as the loop factor, following from the matching condition 
	\begin{equation}
		C_{\phi G}= 0.65\,\frac{\alpha_s}{8 \pi}\, C_{\phi q}
	\end{equation}
}
