...
% and only affected by the trilinear coupling~$C_\phi$ as far as this analysis is concerned.
...
\end{figure}
%####
\par In order to have a preliminary estimate about the sensitivity of Higgs pair production to light Yukawa enhancements, it is important to consider both production and decay effects in terms of signal strength
\begin{equation}
...
%\end{figure}
One~$b$-tagging working point could only constrain either~$ \mu_b$ or~$\mu_c$. In order to resolve the flat direction several $b$-tagging working points~$\left(\epsilon_{c/b}^{\text{$b$-tag}} \right)^2$ are needed. 
%This is illustrated in fig.~\ref{bworkingpoints}, where the working points fitting contours are combined  using Fisher's method~\cite{heard2018choosing}.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=\linewidth]{fig/netzwerk/networks-signal-HL-LHC}
		\caption{Network diagrams of the signal channels of their Pearson correlation ($\rho$) between the features, showing slightly different patterns of correlation amongst these channels.
	}
	\label{fig:cor-net1}
\end{figure}
\begin{figure}[h!]
	...
	\caption{Pearson correlations plotted against each other for the different signals. The coloured and annotated variable pairs points indicate that the difference between the correlation passes the hypothesis testing   FDR-corrected $p$-value threshold~($p<0.01$).
	}
	\label{fig:cor-net2}
\end{figure}
...
\end{table}
For up- and down-quark $ \qqA$, the same matrices were constructed, and since the number of events for these processes scale with $C_{q\phi}^2$, it is only required to produce one confusion matrix for each classification procedure, as for the ggF channel.  For the fitting procedure, a Bayesian framework based on an MCMC method was used, analogous to the procedure described in~\autoref{sec:fit}. 
\par The full analysis code, including the BDT training and fits as well as the confusion matrices for the classification procedures performed can be found in the~\texttt{Github} repository: \href{https://github.com/talismanbrandi/IML-diHiggs.git}{https://github.com/talismanbrandi/IML-diHiggs.git}. 
\begin{figure}[t!]
	\centering
...
\end{table}
\subsection{Second generation bounds}
