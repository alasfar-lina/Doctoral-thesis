...
ggplot(aggrivated) + 
  ...
  scale_x_discrete(labels= unname(TeX(c("$\\kappa_{\\mu}$","$\\kappa_{b}$","$\\kappa_{t}$","$\\kappa_{\\tau}$","$\\kappa_{g}$","$\\kappa_{\\gamma}$","$\\kappa_{W}$","$\\kappa_{Z}$")))) +
  coord_flip() + 
  theme_minimal() + 
...
theme(legend.title = element_text(size = 16)) #+
  #annotate("text", x=4.5, y=0.01, label= "ATLAS+CMS",size=6)+
#annotate("text", x=4., y=0.015, label='Aggregated LHC Run-II',size=4)+
#annotate("text", x=3.65, y=0.01, label=TeX('$L=139+137\\,\\mathrm{fb}^{-1}$'),size=4)
